...
#include "xla/array.h"
#include "xla/hlo/ir/hlo_instruction.h"  // replace service with hlo/ir
#include "xla/hlo/ir/hlo_schedule.h"   // replce service with hlo/ir
// #include "xla/service/hlo_sharding.h"
...
// Support transformation for span.
template <typename T>
std::string SpanToString(absl::Span<T> span) {
  std::ostringstream os;
  os << "[";
...
inline std::string ToStringSimple(const HloSharding& spec) {
  ...
  // spec.tile_assignment().dimensions(): current => absl::span<const int64_t>
  //                                      previous => vector<int64_t>
  // TODO: Support ToString for span<const int64_t>
  return SpanToString(spec.tile_assignment().dimensions());
}
...
inline void ForceOperandSharding(HloInstruction* inst, int operand_num,
  ...
  auto status = inst->ReplaceOperandWith(operand_num, replace_with);
}
