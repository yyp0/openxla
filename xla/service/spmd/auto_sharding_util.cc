...
HloSharding BroadcastSharding(const HloSharding& input_spec,
  ...
  // absl::Span<const int64_t> new_tile_assignment = input_spec.tile_assignment();
  TileAssignment new_tile_assignment = input_spec.tile_assignment();
  // TF_CHECK_OK(new_tile_assignment.Reshape(target_tile_assignment_dimensions));
  new_tile_assignment = new_tile_assignment.Reshape(target_tile_assignment_dimensions);
...
void RemoveIndivisibleStrategies(std::unique_ptr<StrategyVector>& strategies,
  ...
  for (size_t i = 0; i < strategies->leaf_vector.size(); ++i) {
    ...
    if (!output_spec.IsReplicated()) {
      ...
      // absl::Span<const int64_t> tile_assignment = output_spec.tile_assignment();
      TileAssignment tile_assignment = output_spec.tile_assignment();
...
HloSharding GetReduceScatterOutput(const HloInstruction* ins,
  ...
  if (ins->opcode() == HloOpcode::kDot) {
    ...
    if (strategy.output_sharding.IsReplicated()) {
      ...
      // absl::Span<const int64_t> tile_assignment = 
      TileAssignment tile_assignment = 
          strategy.output_sharding.tile_assignment();
      // TF_CHECK_OK(tile_assignment.Reshape({cluster_env.total_devices}));
      tile_assignment = tile_assignment.Reshape({cluster_env.total_devices});
...
// Support TileAssignment.
std::vector<int64_t> GetValuesAlongOneDim(TileAssignment& tile_assignment,
                                          int dim) {
  std::vector<int64_t> ret;
  std::vector<int64_t> indices(tile_assignment.num_dimensions(), 0);

  for (int i = 0; i < tile_assignment.dim(dim); ++i) {
    indices[dim] = i;
    ret.push_back(tile_assignment(indices));
  }
...
bool IsValidTileAssignment(const HloSharding& spec) {
  ...
  // Adapted to new changes of hlo_sharding.
  // Check all tile dims
  // const Array<int64_t>& tile_assignment = spec.tile_assignment();
  // absl::Span<const int64_t> tile_assignment = spec.tile_assignment();
