...
void HardcodeMesh() {
  ...
  current_context["auto_sharding::device_mesh_alpha"] = absl::any(std::move(device_mesh_alpha));

  std::vector<double> device_mesh_beta = {0.1, 0.1};
  current_context["auto_sharding::device_mesh_beta"] = absl::any(std::move(device_mesh_beta));

  current_context["auto_sharding::device_mesh_prof_result"] = absl::any(py::cast<py::object>(py::none()));
}
